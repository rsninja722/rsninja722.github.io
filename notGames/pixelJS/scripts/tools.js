function switchTool(toolName) {
    if (toolName !== "selection") {
        if (toolData.selection.mode === "selected") {
            toolData.selection.mode = "off";
            toolData.selection.startPos = { x: 0, y: 0 };
            toolData.selection.endPos = { x: 0, y: 0 };
            drawSelectContent(0, 0, "confirm");
        }
    }
    tool = toolName;
    hideAllTools();
...
function drawMirrorAxis() {
    if(mirrorMode) {
        var xoffset = projectInfo.w%2 ? 1 : 0;
        var yoffset = projectInfo.h%2 ? 1 : 0;
        var xLine = reverseMousePos({ x: mirror.x.pos + 0.5 + xoffset, y: 0 + yoffset });
        var yLine = reverseMousePos({ x: 0 + xoffset, y: mirror.y.pos + 0.5 + yoffset });
        var topLeft = reverseMousePos({ x: 0 + xoffset, y: 0 + yoffset });
        var bottomRight = reverseMousePos({ x: projectInfo.w + xoffset, y: projectInfo.h + yoffset });
        var mx = mirror.x.on;
        var my = mirror.y.on;
        if (mx) { rect2(xLine.x - 2, topLeft.y, 2, bottomRight.y - topLeft.y, "#666666"); }
        if (my) { rect2(topLeft.x, yLine.y - 2, bottomRight.x - topLeft.x, 2, "#666666"); }
        if (mx) { rect2(xLine.x, topLeft.y, 2, bottomRight.y - topLeft.y, "#252525"); }
        if (my) { rect2(topLeft.x, yLine.y, bottomRight.x - topLeft.x, 2, "#252525"); }
    }
}
...
function handleTools() {
    ...
    function putPixel(x, y, w, h) {
        plot(x, y, w, h);
        // mirroring
        var mx = mirror.x.on && mirrorMode;
        var my = mirror.y.on && mirrorMode;
        if (mx && my) {
            plot(Math.round(x + (mirror.x.pos - x) * 2), y, w, h);
            plot(x, Math.round(y + (mirror.y.pos - y) * 2), w, h);
            plot(Math.round(x + (mirror.x.pos - x) * 2), Math.round(y + (mirror.y.pos - y) * 2), w, h);
        } else if (mx) {
            plot(Math.round(x + (mirror.x.pos - x) * 2), y, w, h);
        } else if (my) {
            plot(x, Math.round(y + (mirror.y.pos - y) * 2), w, h);
...
function getMouse(accuracy=1) {
    let m = mousePosition();
    let offset = 0;
    if (tool === "pen" && toolData.pen.size % 2 === 0) { offset = -0.5; }
    m.x = Math.round(accuracy*(m.x - (projectInfo.w % 2 ? 1 : 0.5) - offset))/accuracy;
    m.y = Math.round(accuracy*(m.y - (projectInfo.h % 2 ? 1 : 0.5) - offset))/accuracy;
    return m;
}