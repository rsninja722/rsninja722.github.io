...
function kdown(e) {
    if(document.activeElement.id===""||document.activeElement.id==="inputEater") {
        var h=e.keyCode;
...
document.body.onmousedown = function() {
    if(document.activeElement.id!=="name"){selectCount=0;}
}
...
function blurAll() {
    ...
    for(let i=0;i<keys.length;i++) {
        numbers[keys[i]].elem.blur();
    }
    keys = Object.keys(colorInput);
    for(let i=0;i<keys.length;i++) {
        colorInput[keys[i]].blur();
    }
