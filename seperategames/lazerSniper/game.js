...
function clearScreen() {
    ...
    //screenData.data=clearScreenCache.data.slice(0,clearScreenCache.data.length);
    for(var dpos=0,l=screenData.data.length;dpos<l;dpos++) {
        screenData.data[dpos]=dpos%4==3?255:255;
    }
    //for(let i=3;i<screenData.data.length;i+=4) {screenData.data[i]=255;}
...
function drawTransformedSprite(sprite,xpos,ypos,rot,isgun) {
    var mat = [[1,0,0],[0,1,0],[0,0,1]];
    Rotate(mat,rot);
    var mat2 = [[1,0,0],[0,1,0],[0,0,1]];
    if(isgun) {
        Translate(mat2,-18,-10);
    } else {
        Translate(mat2,0,-5);
    }
    ...
    let ghhh = sprite.h;
    let gwww = sprite.w;
    for(let x = ~~sx<0?0:~~sx; x < ex&&x<800; x++)
    {
        for(let y = ~~sy<0?0:~~sy; y < ey&&y<600; y++)
        {
            Forward(mat, x, y);
            /*olc::Pixel p = sprCar->GetPixel((int32_t)(nx + 0.5f), (int32_t)(ny + 0.5f));
            Draw(x, y, p);*/
            
            if(ty<ghhh&&tx<gwww&&tx>0&&ty>0) {
                if(sprite.data[~~ty][~~tx]!=0) {
                    screenGrid[y][x]=sprite.data[~~ty][~~tx];
                }
            }
        }
    }
